Attribute VB_Name = "StockAnalysis_AllSheets"
...
'as wel as a wrapper loop to perform action on all worksheets in a workbook

Dim firstOpen As Double, lastClose As Double, sumVol As Double
Dim i As Integer 'used for loop to move through worksheets in workbook
Dim sumRow As Integer
Dim wkstCnt As Integer

wkstCnt = ThisWorkbook.Worksheets.Count

 For i = 1 To wkstCnt
 
  Worksheets(i).Select
  
 'reset the vars that will be used in the worksheet loops
  sumVol = 0
  sumRow = 1
